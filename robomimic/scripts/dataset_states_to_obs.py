...
def extract_trajectory(
    ...
    # Debug num points in point cloud
    # print(np.min([a.shape[0] for a in traj["obs"]["pcd_rgb"]]))
    # print(len(set([a.shape[0] for a in traj["obs"]["pcd_rgb"]])))
    ...
    for k in traj:
        ...
        # # Point clouds have different shapes at each timestep, so don't convert any obs to numpy arrays
        # if not isinstance(traj[k], dict):
        #     traj[k] = np.array(traj[k])
...
def dataset_states_to_obs(args):
    ...
    for ind in tqdm(range(len(demos))):
        ...
        for k in traj["obs"]:
            ...
            # For variable number of point clouds
            # compression = "gzip" if args.compress else None
            #
            # obs_grp = ep_data_grp.create_group("obs/{}".format(k))
            #
            # for t in range(len(traj["obs"][k])):
            ...
            #
            # if not args.exclude_next_obs:
            #     next_obs_grp = ep_data_grp.create_group("next_obs/{}".format(k))
            #
            #     for t in range(len(traj["next_obs"][k])):
            #         next_obs_grp.create_dataset(
            #             f"{t}",
            #             data=traj["next_obs"][k][t],
            #             compression=compression,
            #         )
